...
import com.joins.kidcenter.service.AccountService
...
class AccountController @Autowired constructor(
    ...
    fun create(response: HttpServletResponse, @RequestBody account: AccountDto) {
        SecurityUtil.checkPermission(Permission.paymentsModify)
        response.sendObject(objectMapper, service.save(account))
    }

    @RequestMapping(path = arrayOf("/{id}"), method = arrayOf(RequestMethod.PUT))
    fun modify(response: HttpServletResponse, @PathVariable id: Long, @RequestBody account: AccountDto) {
        SecurityUtil.checkPermission(Permission.paymentsModify)
        response.sendObject(objectMapper, service.save(account.apply { this.id = id }))
    }
