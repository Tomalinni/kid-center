...
import com.joins.kidcenter.dto.LessonTemplateDto
...
@RestController
@RequestMapping("data/lessonTemplates")
class LessonTemplateController @Autowired constructor(
        val service: LessonTemplateService,
    ...
    fun findOne(response: HttpServletResponse, @PathVariable id: Long) {
        SecurityUtil.checkPermission(Permission.lessonTemplatesRead)
        val template: LessonTemplateDto? = service.findOne(id)
        response.sendObject(objectMapper, template)
    }
    ...
    fun create(response: HttpServletResponse, @RequestBody template: LessonTemplateInitDto) {
        SecurityUtil.checkPermission(Permission.lessonTemplatesModify)
        return response.sendSaveResult(objectMapper, service.init(template))
    }

    @RequestMapping(path = arrayOf("/{id}"), method = arrayOf(RequestMethod.PUT))
    fun modify(response: HttpServletResponse, @PathVariable id: Long, @RequestBody template: LessonTemplateSaveDto) {
        SecurityUtil.checkPermission(Permission.lessonTemplatesModify)
        template.id = id
        response.sendSaveResult(objectMapper, service.save(template))
    }
