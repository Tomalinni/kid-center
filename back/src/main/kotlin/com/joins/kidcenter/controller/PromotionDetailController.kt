...
import com.joins.kidcenter.service.PromotionDetailService
...
@RestController
@RequestMapping("data/promotionDetails")
class PromotionDetailController @Autowired constructor(
        val service: PromotionDetailService,
    ...
    fun create(response: HttpServletResponse, @RequestBody promotionDetail: PromotionDetailDto) {
        SecurityUtil.checkPermission(Permission.studentsModify)
        response.sendObject(objectMapper, service.save(promotionDetail))
    }

    @RequestMapping(path = arrayOf("/{id}"), method = arrayOf(RequestMethod.PUT))
    fun modify(response: HttpServletResponse, @PathVariable id: Long, @RequestBody promotionDetail: PromotionDetailDto) {
        SecurityUtil.checkPermission(Permission.studentsModify)
        response.sendObject(objectMapper, service.save(promotionDetail.apply { this.id = id }))
    }
