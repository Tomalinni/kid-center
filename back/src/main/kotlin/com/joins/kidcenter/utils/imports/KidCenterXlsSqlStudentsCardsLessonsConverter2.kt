...
import com.joins.kidcenter.utils.imports.KidCenterXlsSqlStudentsCardsLessonsConverter2.Companion.bookedLessons
import com.joins.kidcenter.utils.imports.KidCenterXlsSqlStudentsCardsLessonsConverter2.Companion.existingRegularBusinessIdsToIds
import com.joins.kidcenter.utils.imports.KidCenterXlsSqlStudentsCardsLessonsConverter2.Companion.hibernateSeq
import com.joins.kidcenter.utils.imports.KidCenterXlsSqlStudentsCardsLessonsConverter2.Companion.templateLessonSlots
...
class KidCenterXlsSqlStudentsCardsLessonsConverter2 {

    companion object {
        @JvmStatic fun main(args: Array<String>) {
            KidCenterXlsSqlStudentsCardsLessonsConverter2().convert()
        }
    ...
    val sqlFilePath = "data/kids-cards-lessons-2.sql"
    ...
    private fun readStudent(container: ItemContainer): KcstlImportStudentRow2 {
        val studentRow = KcstlImportStudentRow2(container.currentCoordinate.row)
    ...
    private fun fillDbData() {
        JdbcConnectionManager().doWithConnection { connection ->
            ...
            selectStudentIds(connection, {
                existingRegularBusinessIdsToIds.put(it.first, it.second)
            })
    ...
    private fun selectStudentIds(connection: Connection, action: (Pair<String, Long>) -> Unit) {
        connection.createStatement().use { statement ->
            statement.executeQuery("select businessid, id from student").use { resultSet ->
                while (resultSet.next()) {
                    action.invoke(Pair(resultSet.getString(1), resultSet.getLong(2)))
                }
            }
        }
    }
...
class KcstlImportStudentRow2(var rowNumber: Int = 0,
    ...
    fun preprocess(): KcstlImportStudentRow2 =
            KcstlImportStudentRow2(
                    rowNumber,
    ...
    fun isValid(): Boolean {
        var valid = true
        if (regularBusinessId.isNotBlank() && !regularBusinessId.matches(Regex(regularBusinessIdPattern))) {
        ...
        if (trialBusinessId.isNotEmpty()) {
            ...
            if (existingRegularBusinessIdsToIds[regularBusinessId] == null) {
                reportWarning("Invalid regular business id $regularBusinessId, should exist in database")
                valid = false
            }
        }
    ...
    fun toSqlInsertStatement(): String {
        ...
        val studentCardSeq = KidCenterXlsSqlStudentsCardsLessonsConverter2.studentCardSeq
        ...
        if (studentId != null) {
            ...
            val totalCardLessons = card1SoldLessons.toInt(0) + card1FreeLessons.toInt(0) + card2SoldLessons.toInt(0) + card2FreeLessons.toInt(0) + card3SoldLessons.toInt(0) + card3FreeLessons.toInt(0)
            if (visitsMap.size() > totalCardLessons) {
    ...
    private fun getStudentId(): Long? {
        val studentId = existingRegularBusinessIdsToIds[regularBusinessId]
        if (studentId == null) {
            println("Not found student businessId $regularBusinessId")
        }
        return studentId
    }
    ...
    private fun tryAppendStudentCard(sqlBuilder: StringBuilder,
                                     ...
                                     price: Int) {
        if (id != null) {
            sqlBuilder.append("insert into public.studentcard (id, student_id, visitType, activationDate, purchaseDate, durationDays, lessonsLimit, lessonsAvailable, price, cancelslimit, cancelsavailable, lateCancelsLimit, lateCancelsAvailable, lastMomentCancelsLimit, lastMomentCancelsAvailable, undueCancelsLimit, undueCancelsAvailable, suspendslimit, suspendsavailable) ")
                    .append("values ($id, $studentId, ${visitType.ordinal}, ${str(sqlDate(purchaseDate))}, ${str(sqlDate(purchaseDate))}, $durationDays, $lessonsLimit, $lessonsAvailable, $price, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50);\n")
