...
open class LessonsVisitTest {
    ...
    @Test
    fun testVisitSingleLessonInPast() {
        ...
        val visitResponse = lessonSlotService!!.visitLesson(StudentSlotRequest(studentSlotId!!))

        LessonIdsResponseAssert(visitResponse).success()
    ...
    @Test
    fun testVisitRepeatedInvocation() {
        val student = studentService!!.save(factories!!.student.student()).obj!!
        val card = cardService!!.save(factories!!.card.regular()).obj!!
        val studentCard = studentCardService!!.save(factories!!.studentCard.fromStudentAndCard(student, card))
        lessonTemplateService!!.save(factories!!.lesson.lessonTemplate()
        ...
        LessonIdsResponseAssert(visitResponse)
                .error("common.error.lessons.change.lesson.status.illegal.status", arrayOf(StudentSlotId(lessonIdToPlan, modifiedStudent).id, StudentSlotStatus.visited))
        StudentSlotAssert(studentSlotService!!.findByLessonAndCard(lessonIdToPlan, studentCard.obj!!.id!!)!!)
                .student(student.id!!)
                .visitType(VisitType.regular)
                .status(StudentSlotStatus.visited)
    ...
    fun testVisitIllegalSlotId() {
        ...
        val visitResponse = lessonSlotService!!.visitLesson(StudentSlotRequest(0))

        LessonIdsResponseAssert(visitResponse)
                .error("common.error.lessons.change.lesson.status.slot.not.found", arrayOf(0L))
        StudentSlotAssert(studentSlotService!!.findByLessonAndCard(lessonIdToPlan, studentCard.obj!!.id!!)!!)
                .student(student.id!!)
                .visitType(VisitType.regular)
    ...
    @Test
    fun testVisitIllegalLessonDate() {
        val student = studentService!!.save(factories!!.student.student()).obj!!
        val card = cardService!!.save(factories!!.card.regular()).obj!!
        val studentCard = studentCardService!!.save(factories!!.studentCard.fromStudentAndCard(student, card))
        lessonTemplateService!!.save(factories!!.lesson.lessonTemplate()
        ...
        LessonIdsResponseAssert(visitResponse)
                .error("common.error.lessons.change.lesson.status.not.started", arrayOf(lessonIdToPlan.id))
        StudentSlotAssert(studentSlotService!!.findByLessonAndCard(lessonIdToPlan, studentCard.obj!!.id!!)!!)
                .student(student.id!!)
                .visitType(VisitType.regular)
                .status(StudentSlotStatus.planned)
    ...
    @Test
    fun testVisitIllegalLessonStatus() {
        val student = studentService!!.save(factories!!.student.student()).obj!!
        val card = cardService!!.save(factories!!.card.regular()).obj!!
        val studentCard = studentCardService!!.save(factories!!.studentCard.fromStudentAndCard(student, card))
        lessonTemplateService!!.save(factories!!.lesson.lessonTemplate()
        ...
        LessonIdsResponseAssert(visitResponse)
                .error("common.error.lessons.change.lesson.status.already.closed", arrayOf(lessonIdToPlan.id))
        StudentSlotAssert(studentSlotService!!.findByLessonAndCard(lessonIdToPlan, studentCard.obj!!.id!!)!!)
                .student(student.id!!)
                .visitType(VisitType.regular)
                .status(StudentSlotStatus.visited)
