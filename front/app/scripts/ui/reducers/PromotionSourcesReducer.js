...
const reducers = new ReducersMap(getInitialState())
    .add({type: 'ajaxFinishedSuccess', entity: 'promotionSources', operation: 'save'},
        (state, action) => {
            return Utils.extend(state, {
                entities: Utils.arr.put(state.entities, action.response, Utils.obj.id)
            });
        })
    .add({type: 'ajaxFinishedSuccess', entity: 'promotionSources', operation: 'delete'},
        (state, action) => {
            return Utils.extend(state, {
...
function PromotionSourcesReducer(state = getInitialState(), action) {
    return reducers.reduce(state, action);
}

module.exports = PromotionSourcesReducer;
